...
describe('Post e2e test', () => {
    ...
    it('should load Posts', async () => {
        ...
        expect(await postComponentsPage.getTitle()).to.eq('jhipsterApp.post.home.title');
    });

    it('should load create Post page', async () => {
        await postComponentsPage.clickOnCreateButton();
        postUpdatePage = new PostUpdatePage();
        expect(await postUpdatePage.getPageTitle()).to.eq('jhipsterApp.post.home.createOrEditLabel');
    ...
    it('should create and save Posts', async () => {
        ...
        await postComponentsPage.clickOnCreateButton();
        await promise.all([
            postUpdatePage.setNameInput('name')
            // postUpdatePage.tagSelectLastOption(),
        ]);
        expect(await postUpdatePage.getNameInput()).to.eq('name');
    ...
    it('should delete last Post', async () => {
        ...
        expect(await postDeleteDialog.getDialogTitle()).to.eq('jhipsterApp.post.delete.question');
